...
begin
  ...
  bmp := new Bitmap(pictureBox1.Width, pictureBox1.Height);
  p := new Pen(Color.White, SizeB);
  g := Graphics.FromImage(bmp);
  g.SmoothingMode := SmoothingMode.HighQuality;
  g.InterpolationMode := InterpolationMode.HighQualityBicubic;
    //g.PixelOffsetMode:=PixelOffsetMode.HighQuality;
  g.FillRectangle(p.Brush, new Rectangle(0,0,PictureBox1.Width,PictureBox1.Height));
  ...
  case bType of
    'brush':
      begin
        ...
        try gp.AddLine(x_coord, y_coord, e.x, e.y); except end;
        x_coord := e.x;
        y_coord := e.y;  
...
if e.Button = System.Windows.Forms.MouseButtons.Left then begin
  ...
  case bType of
    'brush':
      begin
        g.FillEllipse(p.brush, e.x - sizeb div 2, e.y - sizeb div 2, sizeb, sizeb);
        gp.AddLine(x_coord, y_coord, e.x, e.y);
        x_coord := e.x;
        y_coord := e.y;  
        if x_coord1 >= e.x then x_coord1 := e.x;
        if y_coord1 >= e.y then y_coord1 := e.y;
    ...
    'pipette':
        if not FS then FColor := bmp.GetPixel(e.X, e.Y) else SColor := bmp.GetPixel(e.X, e.Y);
    'fill':
      begin
        {try
